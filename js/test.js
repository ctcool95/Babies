...
function calculatePercentiles () {	
	...
	var jsAge = parseFloat(age);
	var jsWeight = parseFloat(weight);
	var jsHeight = parseFloat(height);
	
	if (jsAge>0 && sex) {
		if(jsAge<1 || jsAge>24) {
			console.log("Please enter an age between 1 and 24 months.");
            console.log(jsAge);
			} else {
				...
				console.log(`Your ${sexValue} is in the ${weightPerc}th percentile for weight and ${heightPerc}th percentile for height`)
				console.log(`The average weight is ${weightMean} pounds and average height is ${heightMean} inches for ${sexValue}s that are ${jsAge} months old`);
			}
	} else {
